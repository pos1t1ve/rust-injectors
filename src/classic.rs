...
use windows::Win32::System::Threading::{WaitForSingleObject, CreateRemoteThread, LPTHREAD_START_ROUTINE};
...
pub fn injector(process_name:&str, dll_location: &str){
    ...
    let start_routine: LPTHREAD_START_ROUTINE = unsafe{ std::mem::transmute(proc_address) };
    ...
    if process_thread.is_invalid(){
        ...
        unsafe { VirtualFreeEx(proc_handle, adress, 0, MEM_RELEASE) };
        unsafe { CloseHandle(proc_handle) };
        panic!("{:?}", "[-] Error: failed to create remote thread");
    }
